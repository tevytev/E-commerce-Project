...
const getCart = async (req, res) => {
  const oAuthProvidedId = req?.session?.passport
    ? req.session.passport.user.providedId
    : null;
        ...
        if (repeatProductCheck) {
          ...
          if (repeatedItemWithSize) {
            await repeatedItemWithSize.update({
              quantity:
                Number(repeatedItemWithSize.quantity) + Number(productQuantity),
              price:
                Number(repeatedItemWithSize.price) +
          ...
          await cart.addStock(productStock, {
            through: {
              productName: product.name,
              price: product.price * productQuantity,
              salePrice:
                product.salePrice !== null
                  ? product.salePrice * productQuantity
                  : null,
              quantity: productQuantity,
              size: productSize,
              gender: product.gender,
      ...
      if (productStock) {
        ...
        await cartStock.update({
          ...
          salePrice:
            product.salePrice === null
              ? null
              : Number(product.salePrice) * Number(newProductQuantity),
        });
    ...
    if (order) {
      for (const obj of cart.products) {
        ...
        await order.addProduct(product, {
          through: { quantity: CartProduct.quantity },
        });
